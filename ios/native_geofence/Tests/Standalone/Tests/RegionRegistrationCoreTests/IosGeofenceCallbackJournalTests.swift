...
final class IosGeofenceCallbackJournalTests: XCTestCase {
    ...
    func testEventSurvivesNewJournalAndResumesWithStableIdentity() {
        ...
        XCTAssertEqual(
            restored.complete(eventId: event.eventId, succeeded: false, nowMillis: 1_000),
            .retryAfter(milliseconds: 1_000)
        )
    ...
    func testRetryExhaustionIsAnIntentionalTerminalDisposition() {
        ...
        XCTAssertNotNil(journal.beginAttempt(eventId: event.eventId, nowMillis: 1_000))
        XCTAssertEqual(
            journal.complete(eventId: event.eventId, succeeded: false, nowMillis: 1_000),
            .retryAfter(milliseconds: 1_000)
        )
        XCTAssertNotNil(journal.beginAttempt(eventId: event.eventId, nowMillis: 2_000))
        XCTAssertEqual(
            journal.complete(eventId: event.eventId, succeeded: false, nowMillis: 2_000),
            .terminallyDiscarded
        )
